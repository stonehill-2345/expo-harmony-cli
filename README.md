...
`pnpm start:harmony` 会启动 8888 端口的 Harmony Metro，自动尝试执行 `hdc rport tcp:8888 tcp:8888` 和 `hdc rport tcp:8081 tcp:8888`，并输出局域网地址。真机仍无法加载 bundle 时，在 RNOH Dev Settings 中填写终端输出的 `<局域网 IP>:8888`；也可设置 `HARMONY_METRO_HOST=<局域网 IP>` 显式指定地址。
