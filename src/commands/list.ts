...
export async function list(_args: string[] = []): Promise<void> {
  const sdk = detectSdkVersion(process.cwd());
  const compatTable = getCompatTable(sdk);
  console.log(`compat-table（鸿蒙兼容表，${sdk}）：\n`);
  for (const [pkg, entry] of Object.entries(compatTable)) {
