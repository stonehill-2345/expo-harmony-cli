...
export function validateHarmonyTemplate(templateDir: string, manifest: HarmonyTemplateManifest): void {
  ...
  assertFileContains(templateDir, 'hvigorfile.ts', 'createRNOHProjectPlugin({', 'hvigorfile.ts must use createRNOHProjectPlugin options');
  assertFileContains(templateDir, 'hvigorfile.ts', 'bundler: { enabled: false }', 'hvigorfile.ts must leave release bundling to the CLI');
  assertFileContains(templateDir, 'entry/hvigorfile.ts', 'createRNOHModulePlugin({', 'entry/hvigorfile.ts must use createRNOHModulePlugin');
  assertFileContains(templateDir, 'entry/hvigorfile.ts', 'metro: null', 'entry/hvigorfile.ts must leave Metro management to the CLI');
  assertFileContains(templateDir, 'entry/hvigorfile.ts', 'autolinking: null', 'entry/hvigorfile.ts must leave autolinking management to the CLI');
  assertFileContains(templateDir, 'entry/hvigorfile.ts', "rnohModulePath: './oh_modules/@rnoh/react-native-openharmony'", 'entry/hvigorfile.ts must configure the RNOH codegen module path');
  assertFileContains(templateDir, 'entry/src/main/ets/PackageProvider.ets', 'RNPackage[]', 'PackageProvider.ets must return RNPackage[] for legacy and RNOHPackage compatibility');
  assertFileContains(templateDir, 'entry/src/main/ets/entryability/EntryAbility.ets', 'getRNOHWorkerScriptUrl()', 'EntryAbility.ets must define getRNOHWorkerScriptUrl');
  assertFileContains(templateDir, 'entry/src/main/ets/entryability/EntryAbility.ets', 'entry/ets/workers/RNOHWorker.ets', 'EntryAbility.ets must reference the RNOH worker');
  assertFileContains(templateDir, 'entry/src/main/ets/workers/RNOHWorker.ets', 'setupRNOHWorker({', 'RNOHWorker.ets must initialize the RNOH worker');
  assertFileContains(templateDir, 'entry/src/main/ets/workers/RNOHWorker.ets', 'thirdPartyPackagesFactory: getRNOHPackages', 'RNOHWorker.ets must register third-party packages');
  assertFileContains(templateDir, 'build-profile.json5', '"nativeCompiler": "BiSheng"', 'build-profile.json5 must configure nativeCompiler');
  assertFileContains(templateDir, 'entry/src/main/module.json5', '"name": "ohos.permission.INTERNET"', 'module.json5 must request INTERNET permission');
  assertFileContains(templateDir, 'entry/src/main/module.json5', '"querySchemes"', 'module.json5 must declare querySchemes for Linking');
